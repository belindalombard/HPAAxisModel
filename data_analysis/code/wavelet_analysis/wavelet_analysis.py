...
--------------------------------------------
  Figure 2C  — Wavelet spectrogram for Participant 9 (Cortisol, dt=10 min)
               Saved: data_analysis/output/Cortisol/10min/wavelet_Participant_9_Cortisol.pdf
               Saved: data_analysis/output/Cortisol/10min/wavelet_Participant_9_Cortisol.png

  Figure S3A — Percentile curves (5th–90th) of wavelet power across time, per participant
               (periods 10–300 min, Cortisol, dt=10 min). 2×5 grid, one subplot per participant;
               coloured lines for each percentile, no legend.
               Generated by: plot_participant_percentile_power()
               Saved: data_analysis/output/Cortisol/10min/wavelet_percentile_power_period_Cortisol.png

  Figure S3B — Total wavelet power (summed across time) as a function of period (10–300 min),
               per participant, for all 10 participants (Cortisol, dt=10 min).
               Generated by: plot_participant_total_power()
               Saved: data_analysis/output/Cortisol/10min/wavelet_total_power_period_Cortisol.png
...
Note that in the manuscript we used dt=10 for Cortisol (the corresponding shifted data file
must be created first via data_prep.py).
...
Input:
    - data_analysis/data/data_shifted_{hormone}_{dt}min_00_00.csv
    - data_analysis/options/participant_options.csv

Output:
    - data_analysis/output/{hormone}/{dt}min/stacked_wavelets_{hormone}.npy
    - data_analysis/output/{hormone}/{dt}min/periods.npy
    - data_analysis/output/{hormone}/{dt}min/times.npy
    - Per-participant wavelet spectrograms and summary plots in the same directory
"""
...
def plot_participant_percentile_power(data, periods, hormone='Cortisol', results_folder='output', save=True):
    """Plot percentile curves of wavelet power across time for each participant (Figure S3A)."""
    percentiles = [5, 25, 50, 75, 90]
    colors = ['#d7191c', '#fdae61', '#a6d96a', '#abd9e9', '#2c7bb6']
    ...
    with ppl.rc_context({
        ...
        for i, participant_data in enumerate(data):
            ax = axs[i // 5, i % 5]
            # participant_data shape: (num_periods, num_timepoints)
            # percentile_curves shape: (num_percentiles, num_periods)
            percentile_curves = np.percentile(participant_data, percentiles, axis=1)
            for j, curve in enumerate(percentile_curves):
                ax.plot(periods[i], curve, color=colors[j])
            ax.set_title(f'Participant {i + 1}')
        ...
        if save:
            ppl.savefig(f'{results_folder}/wavelet_percentile_power_period_{hormone}.png', dpi=300, bbox_inches='tight')
            ppl.close()
...
def main():
    ...
    plot_participant_percentile_power(stacked_wavelet, participant_periods, hormone, results_folder)
