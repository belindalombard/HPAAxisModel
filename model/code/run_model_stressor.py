...
import argparse
import sys
from additional_functions.old import model_plotting_functions
from additional_functions import metrics_calculations as mc
import csv
from additional_functions import additional_functions as af
...
def build_output_dir(scenario: str, stressor_parameters: dict) -> str:
    """Return output directory path under stressor_output/<scenario>/<param_string>."""
    param_str = (
    ...
    base_dir = ".." if os.path.basename(os.getcwd()) == "code" else "."
    return os.path.join(base_dir, "stressor_output", scenario, param_str)
...
def parse_arguments():
    ...
    return {'config_file': f'config/{args.config}'}, stressor_parameters, args.scenario, args.save, args.no_legend, args.crh_source, args.plot_days, args.max_minutes_plot
...
config_file_relative = args['config_file']
# If running from code/, go up one level to find config/
if not os.path.exists(config_file_relative):
    config_file = os.path.join('..', config_file_relative)
else:
    config_file = config_file_relative
