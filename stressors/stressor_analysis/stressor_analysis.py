"""Stressor Analysis — Figure S6B and S6C

Provides functions to analyse and plot individual CORT perturbation responses,
including AUC changes and peak-difference annotations. Called by other stressor
scripts (e.g. run_model_stressor.py) when plotting individual stressor examples.

Manuscript figures generated using functions in this module
-----------------------------------------------------------
  Figure S6B — Example CORT response (orange) to a short stressor perturbation at
               one circadian/ultradian phase. Shaded AUC change (orange/red) and
               peak-difference arrow shown. Grey line = perturbed CRH signal.

  Figure S6C — Second example stressor perturbation at a different phase, same
               annotation style as S6B.

               Both panels are produced by calculate_auc_after_perturbation() and
               calculate_peak_diff_after_perturbation() when called with fig != None.
"""
...
def filter_stressor_data(magnitude=None, duration=None, time_in_scope=None, filename='../output/ACUTE/results.csv'): 
    import pandas as pd
...
for i in range(len(all_directories)): 
    ...
    base = os.path.basename(os.path.normpath(all_directories[i]))
    ...
    try:
        ...
        with open('points_to_consider.json', "r") as file:
...
for i in range(len(all_directories)): 
    base = os.path.basename(os.path.normpath(all_directories[i]))
    magnitude, duration, timing_abs = parse_directory_name(base)
    # points_to_consider.json is generated by get_stressor_times.py and lives in this directory
    with open('points_to_consider.json', "r") as file:
        data = json.load(file)
...
print(metrics_df)
import os
results_path = '../output/ACUTE/results.csv'
